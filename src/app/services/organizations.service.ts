...
export class OrganizationsService {
  ...
  getAllUsers(): Observable<UserWithAvatarModel[]> {
    return this._httpClient.get<UserWithAvatarModel[]>(
  ...
  ): Observable<Record<string, TeamModel[]>> {
    return forkJoin(
      organizationIds.map((organizationIds) =>
        this._httpClient.get<TeamModel[]>(
          `https://636ce2d8ab4814f2b2712854.mockapi.io/organizations/${organizationIds}/teams`
        )
      )
    ).pipe(
      map((organizationTeams: TeamModel[][]) =>
        organizationTeams.reduce(
          (a, c, idx) => ({ ...a, [organizationIds[idx]]: c }),
